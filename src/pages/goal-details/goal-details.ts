...
export class GoalDetailsPage {
	...
	refresh() {
		...
		this.user.refreshGoal(this.goal.slug, () => { this.reload(); });
	}
			...
			buttons: [
				{
					text: 'Direct',
					handler: () => {
						this.user.addDatapointPrompt(this.goal, () => { this.reload(); });
					}
				},
				{
					text: 'Timer',
	...
	editDatapointPrompt(datapoint: any) {
		this.user.editDatapointPrompt(this.goal, datapoint, () => { this.reload(); });
	}
